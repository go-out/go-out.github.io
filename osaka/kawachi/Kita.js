'use strict'

const kawachiKita = {
    'type': 'FeatureCollection',
    'features': [
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.68995391168988, 34.86543099924788]
            },
            'properties': {
                'title': '鏡伝池緑地（枚方市市民の森）',
                'address': '「緑と太陽と文化のまち」のスローガンのもと枚方市市政35年を記念して造成された地域密着型の緑地公園',
                'link': null,
                'tag': ['park'],
                'zoom': 15.5
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.66767547965816, 34.84186768653112]
            },
            'properties': {
                'title': '片埜神社',
                'address': '交野郡の産土社 境内の北に牧野公園が隣接',
                'link': null,
                'tag': ['legacy'],
                'google': 'https://maps.app.goo.gl/5kDKpodumzR2BETVA',
                'zoom': 17.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [10],
                    'date': null,
                    'note': [
                        '<b>例祭</b>'
                    ],
                    'thisYear': [
                        {
                            'year': 'every',
                            'date': [15],
                            'time': '10:00 湯立て神事 | 19:00 夜神楽（吉田流神楽・笠井社中）',
                            'note': null
                        }
                    ]
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.6805770289251, 34.854128516577944]
            },
            'properties': {
                'title': '二ノ宮神社',
                'address': '3000坪を越す広大な社叢は枚方市の保存樹林に指定 別称「河内の牛頭（ごず）さん」',
                'link': null,
                'tag': ['legacy'],
                'google': 'https://maps.app.goo.gl/ftUUXnfLEWec7NyE6',
                'zoom': 17.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [10],
                    'date': null,
                    'note': [
                        '<b>例祭</b>'
                    ],
                    'thisYear': [
                        {
                            'year': 'every',
                            'date': [15],
                            'time': null,
                            'note': null
                        }
                    ]
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.72891432715738, 34.79464776400789]
            },
            'properties': {
                'title': '三之宮神社',
                'address': '一ノ宮（片埜神社）・二ノ宮（船橋本町）とともに大坂城の鬼門除けとして再興された神社',
                'link': null,
                'tag': ['legacy'],
                'google': 'https://maps.app.goo.gl/SomzvQHhjwutLWXn8',
                'zoom': 17.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [10],
                    'date': null,
                    'note': [
                        '<b>例祭</b>'
                    ],
                    'thisYear': [
                        {
                            'year': 'every',
                            'date': [15],
                            'time': null,
                            'note': null
                        }
                    ]
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.69117026605693, 34.821585842302994]
            },
            'properties': {
                'title': '山田池公園',
                'address': '1200年前の築造といわれるため池「山田池」を中心に整備された広大な公園',
                'link': null,
                'tag': ['park'],
                'zoom': 14.5
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.65399897980524, 34.79119198561476]
            },
            'properties': {
                'title': '香里ケ丘',
                'address': 'かつて「東洋一のマンモス団地」と呼ばれたニュータウン',
                'link': "index/?id=osaka-kawachi&area=korigaoka",
                'tag': ['feature'],
                'zoom': 14.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [4],
                    'date': '例年4月上旬、香里ケ丘中央公園・こもれび水路などの桜が見頃を迎え、日本庭園「以楽公園」が 1~2週間のみ（10~16時）一般開放される',
                    'note': null,
                    'thisYear': null
                },
                {
                    'title': null,
                    'month': [11],
                    'date': '例年11月中旬から下旬、いちょう・けやき通りの紅葉が見頃を迎え、日本庭園「以楽公園」が 1~2週間のみ（10~16時）一般開放される',
                    'note': null,
                    'thisYear': null
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.70956607960892, 34.78653228009544]
            },
            'properties': {
                'title': '交野山',
                'address': '源氏の滝 ~ せせらぎの道 ~ 山頂（観音岩）~ やまかぜの小路<br>生駒山地の北端 頂上に観音岩と呼ばれる巨岩が一つそびえる珍しい山',
                'link': null,
                'tag': ['nature'],
                'zoom': 15
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.6931858496559, 34.74780450810381]
            },
            'properties': {
                'title': '磐船神社',
                'address': '高さ12メートルにもなる巨大な磐座「天の磐船」を御神体とする聖地',
                'link': "spot/?id=osaka&area=kawachi&name=iwafunejinja",
                'tag': ['legacy'],
                'zoom': 17.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [5],
                    'date': null,
                    'note': [
                        '<b>春季例祭</b>'
                    ],
                    'thisYear': [
                        {
                            'year': 'every',
                            'date': [5],
                            'time': null,
                            'note': null
                        }
                    ]
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.70563820800095, 34.7677359812031]
            },
            'properties': {
                'title': 'くろんどの森',
                'address': '京阪私市（きさいち）駅から奈良県の「くろんど池」に至るハイキングコースの大阪側',
                'link': null,
                'tag': ['nature'],
                'zoom': 14.5
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.68525996860348, 34.7526006150014]
            },
            'properties': {
                'title': 'ほしだの森',
                'address': '全長280m・高さ50mの巨大吊橋「星のブランコ」、天孫降臨伝説のある「哮が峰（たけるがみね）」をはじめ、多くの巨石が散在するアクティビティと冒険の森',
                'link': "spot/?id=osaka&area=kawachi&name=hoshida",
                'tag': ['nature'],
                'zoom': 14.5
            },
            'featured': [
                {
                    'title': null,
                    'month': [11, 12],
                    'date': '11月下旬から12月上旬、コナラ・カエデなどの紅葉が見頃を迎える',
                    'thisYear': null
                }
            ]
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.68253222329588, 34.7657540767315]
            },
            'properties': {
                'title': '大阪公立大学附属植物園',
                'address': '植物学の研究を目的に、日本の代表的な11種類の樹林型を再現する植物園',
                'link': null,
                'tag': ['nature'],
                'zoom': 15
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.66823061556045, 34.729016984022834]
            },
            'properties': {
                'title': 'むろいけの森',
                'address': '笑顔が集まる緑育空間。家族で学びにふれる森。',
                'link': null,
                'tag': ['nature'],
                'zoom': 14.5
            }
        },
        {
            'type': 'Feature',
            'geometry': {
                'type': 'Point',
                'coordinates': [135.65360446221968, 34.72682439834762]
            },
            'properties': {
                'title': '飯盛山',
                'address': '野崎観音 - 野崎城跡 - 飯盛山城跡 - 山頂 - 四條畷神社',
                'link': null,
                'tag': ['nature'],
                'zoom': 14
            }
        }
    ]
}